...
export const getDisplayNameFromEntityInstance = (
  ...
  const defaultName = instanceContainer.id
  const displayNamePath = entity.displayName ?? "name"

  if (typeof displayNamePath === "string") {
    return (
      (getValueAtPath(instanceContainer.content, displayNamePath) as string | undefined) ??
      defaultName
    )
  } else {
    const localeMapPath = displayNamePath.pathToLocaleMap ?? "translations"
    const localeMap = getValueAtPath(instanceContainer.content, localeMapPath) as
    ...
    return availableLocales.length === 0
      ? defaultName
      : (getValueAtPath(
          localeMap?.[(locale as LocaleMapKey | undefined) ?? availableLocales[0]!] ??
            localeMap?.[availableLocales[0]!],
          pathInLocaleMap,
        ) as string | undefined) ?? defaultName
  }
}
