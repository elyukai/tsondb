    ...
    <Layout breadcrumbs={[{ url: "/", label: "Home" }]}>
      ...
      <ul class="instances">
        {instances.map(instance => (
          ...
          >
            <h2>
              {getDisplayNameFromEntityInstance(entity.declaration, instance.content, instance.id)}
            </h2>
            <p aria-hidden class="id">
              {instance.id}
            </p>
            <div className="btns">
              <a href={`/entities/${entity.declaration.name}/instances/${instance.id}`} class="btn">
                Edit
              </a>
              <button
                class="destructive"
